...
信息源全部集中在 `feeds.yml`：增删 RSS 只需加一段配置；结构化 API（AniList / Jikan / Bangumi / MangaDex）通过 `apis` 段开关；`classifier` 段的关键词表决定综合类条目如何自动归入四大领域；`site` 段控制条数上限与归档保留天数。
...
本站仅聚合标题、链接与不超过 160 字的摘要，并明确标注出处、点击跳转原文，不转载全文、不搬运图片。抓取频率为每源每日 2 次并设置了 UA 标识与请求间隔。如需商用或扩大抓取范围，请先阅读各源的服务条款与 robots.txt。
